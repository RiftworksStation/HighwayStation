...
/datum/controller/subsystem/ticker/proc/market_report()
	...
	switch(tourist_income)
		if(0)
			parts += "[span_redtext("Обслуживание не заработало [MONEY_NAME]...")]<br>"
		if(1 to 2000)
	...
	if(mr_moneybags)
		parts += "Самым богатым членом экипажа в конце смены был <b>[mr_moneybags.account_holder] с [mr_moneybags.account_balance]</b> [MONEY_NAME]!</div>"
	else
