		...
		if(href_list["hud"] == "s")
			...
			if(ishuman(human_or_ghost_user))
				...
				if(href_list["add_citation"])
					...
					new_citation.alert_owner(usr, src, target_record.name, "Вам был выставлен штраф в размере [fine] [MONEY_NAME_AUTOPURAL(fine)] за '[citation_name]'. Штраф может быть оплачен у отдела безопасности.")
