...
/obj/item/firing_pin/paywall/pin_auth(mob/living/user)
	...
	var/license_request = tgui_alert(user, "Вы готовы к оплате [payment_amount] [MONEY_NAME_AUTOPURAL(payment_amount)] за [( multi_payment ) ? "каждый выстрел из [gun.declent_ru(GENITIVE)]" : "лицензию [gun.declent_ru(GENITIVE)]"]?", "Покупка оружия", list("Да", "Нет"), 15 SECONDS)
