...
/datum/martial_art/cqc/proc/check_block(mob/living/cqc_user, atom/movable/hitby, damage, attack_text, attack_type, ...)
	...
	if(istype(attacker) && cqc_user.Adjacent(attacker))
		cqc_user.visible_message(
			span_danger("[capitalize(cqc_user.declent_ru(NOMINATIVE))] [ru_attack_verb(blocking_text_s)] [attack_text] и выкручивает руку [attacker.declent_ru(GENITIVE)] за [attacker.ru_p_them()] спиной!"),
			span_userdanger("Вы [ru_attack_verb(blocking_text)] [attack_text]!"),
		)
		attacker.Stun(4 SECONDS)
	else
		cqc_user.visible_message(
			span_danger("[capitalize(cqc_user.declent_ru(NOMINATIVE))] [ru_attack_verb(blocking_text_s)] [attack_text]!"),
			span_userdanger("Вы [ru_attack_verb(blocking_text)] [attack_text]!"),
		)
