/obj/item/clothing/under/rank/security/officer/corporate
	name = "security corporate uniform"
	icon = 'modular_content/objects/icons/obj/clothing/under/security.dmi'
	worn_icon = 'modular_content/objects/icons/mob/clothing/under/security.dmi'
	icon_state = "sec_corporate"
	can_adjust = FALSE

/obj/item/clothing/under/rank/security/officer/skirt/corporate
	name = "security corporate skirt"
	icon = 'modular_content/objects/icons/obj/clothing/under/security.dmi'
	worn_icon = 'modular_content/objects/icons/mob/clothing/under/security.dmi'
	icon_state = "sec_corporate_skirt"
	can_adjust = FALSE
