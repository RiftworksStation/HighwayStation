...
/obj/item/clothing/suit/space/hev/equipped(mob/M, slot)
	...
	if(iscarbon(M))
		for(var/voice in funny_signals)
			...
			owner = M
			add_queue('modular_content/objects/sounds/hev/blip.ogg', 2 SECONDS, purge_queue=TRUE)
			add_queue('modular_content/objects/sounds/hev/01_hev_logon.ogg', 11 SECONDS)
			add_queue('modular_content/objects/sounds/hev/04_vitalsigns_on.ogg', 4 SECONDS)
			add_queue('modular_content/objects/sounds/hev/09_safe_day.ogg', 8 SECONDS)
	else
		for(var/voice in funny_signals)
...
/obj/item/clothing/suit/space/hev/proc/handle_death(gibbed)
	SIGNAL_HANDLER
	add_queue('modular_content/objects/sounds/hev/death.ogg', 5 SECONDS, purge_queue=TRUE)
...
/obj/item/clothing/suit/space/hev/proc/handle_ignite(mob/living)
	SIGNAL_HANDLER
	SOUND_BEEP('modular_content/objects/sounds/hev/beep_3.ogg')
	add_queue('modular_content/objects/sounds/hev/heat.ogg', 3 SECONDS)

//Shock
/obj/item/clothing/suit/space/hev/proc/handle_shock(mob/living)
	SIGNAL_HANDLER
	SOUND_BEEP('modular_content/objects/sounds/hev/beep_3.ogg')
	add_queue('modular_content/objects/sounds/hev/shok.ogg', 3 SECONDS)

//Helmet
/obj/item/clothing/head/helmet/hev_helmet
	name = "hazardous environment suit helmet"
	desc = "The Mark IV HEV suit helmet."
	icon = 'modular_content/objects/icons/obj/clothing/head/helmet.dmi'
	worn_icon = 'modular_content/objects/icons/mob/clothing/head/helmet.dmi'
	lefthand_file = 'modular_content/objects/icons/mob/inhands/clothing_left_hand.dmi'
	righthand_file = 'modular_content/objects/icons/mob/inhands/clothing_right_hand.dmi'
